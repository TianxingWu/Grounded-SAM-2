...
SOURCE_VIDEO_FRAME_DIR = f"./temp_frames_GPU{GPU_ID}"
...
for sub_clip_path in tqdm(sub_clip_paths, desc=f"THREAD {ID}/{THREAD_NUM}"):
    try:
        ...
        # read frames for processing
        # vr = VideoReader(uri=video_path)
        # orig_fps = vr.get_avg_fps()
        # ori_vlen = len(vr)
        # frames = vr.get_batch(range(start_idx, min(ori_vlen, end_idx+1)))  # shape: (T, H, W, C)
        """
        ...
        for obj_name in obj_names:
            ...
            inference_state = video_predictor.init_state(video_path=video_dir, offload_video_to_cpu=True, async_loading_frames=True, verbose=False)
            ...
            # prompt grounding dino to get the box coordinates on specific frame
                ...
                if len(mask_dict.labels) == 0:
                    ...
                    for out_frame_idx, out_obj_ids, out_mask_logits in video_predictor.propagate_in_video(inference_state, max_frame_num_to_track=step, start_frame_idx=start_frame_idx, verbose=False):
