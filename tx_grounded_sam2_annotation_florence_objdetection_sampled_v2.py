...
task_prompt = TASK_PROMPT["object_detection"]
...
for sub_clip_path in tqdm(sub_clip_paths, desc=f"THREAD {ID}/{THREAD_NUM}"):
    ...
    for i, frame in enumerate(frames):
        # run florence-2 dense region captioning
        ...
        # remove 'human face' entries from both lists
        filtered = [(box, name) for box, name in zip(input_boxes, class_names) if name != 'human face']
        input_boxes, class_names = zip(*filtered) if filtered else ([], [])
        input_boxes = list(input_boxes)
        class_names = list(class_names)

        for name in class_names:
            if name not in name2id:
                name2id[name] = len(name2id) + 1
        ...
        # refine mask with mask input
        print(logits.shape) # CxHxW
    ...
    """
    output_dir = sub_clip_path.replace('pexelx_st', '/mnt/Text2Video/fanweichen/tx/dataset/mflow/4DGen-Dataset-tx-sampled-100/pexelx_seg_florence_objdetection_v2') # ....clip_xxx-xxx.mp4/
